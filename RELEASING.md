...
- Keep `Cargo.toml` and `child.toml` versions aligned with release tag (`vX.Y.Z`).
...
1. Update `Cargo.toml` and `child.toml` versions.
...
- `.github/workflows/release.yml` runs on `v*` tags and publishes Mother registry assets:
  - `patina_ai_child_slate_manager.wasm`
  - `patina_ai_child_slate_manager.wasm.sha256`
  - `child.toml`
  - `child.toml.sha256`
  - `checksums.txt` containing hashes for both the component and manifest
